...
char * power(char *first_number, char *second_number, int system) {
    ...
    int i = 0;
    char *x=NULL, *y=NULL, *result=NULL, *second_number2=NULL;
    ...
    printf("compare=%d\n", compare(second_number, "0"));
    result = (char *)malloc(new_n);
    if (result == NULL) { 
        printf("UNABLE TO ALLOCATE MEMORY");
        return NULL;
    }
    second_number2 = (char *)malloc(m);
    if (second_number2 == NULL) { 
        printf("UNABLE TO ALLOCATE MEMORY");
        free(result);
        return NULL;
    ...
    printf("second_number2: ");
    print_number(second_number2, 1);
    if(compare(second_number2, "0") == 0) {
        printf("second_number=0 x^0 = 0\n");
        if(result != NULL) free(result);
        printf("len(result)=%d\n", len(result));
        result[0] = '1';
        result[1] = '\0';
        if(second_number2 != NULL) free(second_number2);
        if(y != NULL) free(y);
        printf("free y\n");
        if(x != NULL) free(x);
        printf("free x\n");
    ...
    if (!(compare(second_number2, "1") == 0)) {
        if (mod_2(second_number2, system) == 0) {
            /* x^n = x^(n/2) * x^(n/2) */
            x = div_2(second_number2, system);
            if(second_number2 != NULL) free(second_number2);
            second_number2 = x;
            ...
            for (i = len(second_number2); i >= 0; i--) {
                if(i== 0 && second_number2[i] == '0') {
                    second_number2[0] = '0';
                    continue;
                }
            ...
            x = power(first_number, second_number2, system);
            printf("x: ");
            print_number(x, 1);
